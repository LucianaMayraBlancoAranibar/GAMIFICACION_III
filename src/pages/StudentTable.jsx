...
function StudentTable() {
  ...
  const filteredStudentUsers = studentUser.filter((student) =>
      ...
      axios
        ...
        .then((response) => {
          ...
          setStudentUser((prevStudentUser) =>
            prevStudentUser.filter(
              (studentUser) => studentUser.idStudent !== studentUserToDelete
            )
          );
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="relative p-4 sm:p-6 rounded-sm mb-8">
          ...
          <div className="mr-10 grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <Link to="/StudentForm">
              {" "}
              {/* Enlace a la página de añadir studentUser */}
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-center">
                    <div className="font-semibold text-left">Carrera</div>
                  </th>
                  <th scope="col" className="px-6 py-3 text-center">
                    <div className="font-semibold text-left">Academia</div>
                  </th>
                  <th scope="col" className="px-6 py-3 text-center">
                ...
                {filteredStudentUsers.map((studentUser) => (
                  ...
                  >
                      ...
                      {user.find((u) => u.idUsuario === studentUser.idStudent)
                        ? career.find(
                            (c) =>
                              c.idCareer ===
                              user.find(
                                (u) => u.idUsuario === studentUser.idStudent
                              ).idCareer
                          )?.careerName || "Carrera No Encontrada"
                        : "Usuario No Encontrado"}
                    </td>
                    <td className="px-6 py-4">
                      {user.find((u) => u.idUsuario === studentUser.idStudent)
                        ? academicUnities.find(
                            (a) =>
                              a.idAcademicUnity ===
                              user.find(
                                (u) => u.idUsuario === studentUser.idStudent
                    ...
                    <td className="px-6 py-4 text-left">
                      <Link to={`/StudentEdit/${studentUser.idStudent}`}>
                        {" "}
                      ...
                      <button
                        ...
                        onClick={() =>
                          setStudentUserToDelete(studentUser.idStudent)
                        } // Establece el ID de la facultad para eliminar
                      >
                        <BsTrashFill />
                      </button>
                    </td>
