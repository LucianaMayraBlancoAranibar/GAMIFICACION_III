...
function ManagerTable() {
  ...
  const filteredGestorUsers = gestorUser.filter((gestor) =>
    (gestor.firstName + " " + gestor.lastName)
      .toLowerCase()
      ...
      axios
        ...
        .then((response) => {
          ...
          setGestorUser((prevGestorUser) =>
            prevGestorUser.filter(
              (gestorUser) => gestorUser.idGestor !== gestorUserToDelete
            )
          );
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="relative p-4 sm:p-6 rounded-sm mb-8">
          ...
          <div className="mr-10 grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <Link to="/ManagerForm">
              {" "}
              {/* Enlace a la página de añadir gestorUser */}
          ...
          <div className="my-4">
            <input
              ...
              placeholder="Buscar por nombre del gestor..."
              value={searchTerm}
          ...
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-5 max-h-[600px] overflow-y-auto">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody className="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
                {filteredGestorUsers.map((gestorUser) => (
                  <tr
                    key={gestorUser.idGestor}
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700"
                  >
                    <td className="px-6 py-4">
                      {gestorUser.firstName + " " + gestorUser.lastName}
                    </td>
                    <td className="px-6 py-4">{gestorUser.email}</td>
                    <td className="px-6 py-4 text-left">
                      <Link to={`/ManagerEdit/${gestorUser.idGestor}`}>
                        {" "}
                      ...
                      <button
                        ...
                        onClick={() =>
                          setGestorUserToDelete(gestorUser.idGestor)
