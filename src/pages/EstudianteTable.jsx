...
function EstudianteTable() {
  ...
  useEffect(() => {
    // Realiza una solicitud a tu API para obtener la lista de Estudiante
    axios
  ...
  const handleDeleteSucursal = () => {
    if (EstudianteToDelete) {
      // Realiza una solicitud DELETE a la API para eliminar la Estudiante
      axios
        .delete(`https://localhost:7187/api/Students/${EstudianteToDelete}`)
        .then((response) => {
          // Actualiza la lista de Estudiante después de la eliminación
          setEstudiante((prevSucursal) =>
            prevSucursal.filter((Estudiante) => Estudiante.idDepartment !== EstudianteToDelete)
          );
          setEstudianteToDelete(null); // Restablece el estado
        })
        .catch((error) => {
          console.error(error);
          setEstudianteToDelete(null); // Restablece el estado en caso de error
        });
    }
  };
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="relative p-4 sm:p-6 rounded-sm overflow-hidden mb-8">
          ...
          <div className="mr-10 grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <Link to="/EstudianteForm"> {/* Enlace a la página de añadir Estudiante */}
          ...
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody className="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
              {Estudiante.map((Estudiante) => (
                <tr key={Estudiante.idDepartment} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                
                  <td className="px-6 py-4">{Estudiante.departmentName}</td>

                  <td className="px-6 py-4">{Facultad.find((Facultad) => Facultad.idFaculty === Estudiante.idFaculty)?.facultyName}</td>
                  <td className="px-6 py-4 text-left">
                  <Link to={`/EstudianteEdit/${Estudiante.idDepartment}`}> {/* Redirigir a la página de edición con el ID */}
                    <button
                    ...
                    <button
                     className="px-4 py-4 ml-3 leading-5 text-white transition-colors duration-200 transform bg-red-500 rounded-md hover:bg-red-400 focus:outline-none focus:bg-gray-600"
                      onClick={() => setEstudianteToDelete(Estudiante.idDepartment)} // Establece el ID de la Estudiante para eliminar
                    >
                      Eliminar
                    </button>
                  </td>
          ...
          {EstudianteToDelete && (
            <div className="bg-white p-4 shadow-md rounded-md absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <p>¿Seguro que deseas eliminar esta Estudiante?</p>
              <div className="mt-2">
                <button
                ...
                <button
                  ...
                  onClick={() => setEstudianteToDelete(null)} // Cancelar la eliminación
                >
                  Cancelar
                </button>
              </div>
