...
function DepartamentoTable() {
  ...
  const handleDeleteSucursal = () => {
    if (departamentoToDelete) {
      ...
      axios
        .delete(
          `https://localhost:7205/api/Departments/${departamentoToDelete}`
        )
        .then((response) => {
          ...
          setDepartamento((prevSucursal) =>
            prevSucursal.filter(
              (Departamento) =>
                Departamento.idDepartment !== departamentoToDelete
            )
          );
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="relative p-4 sm:p-6 rounded-sm  mb-8">
          ...
          <div className="mr-10 grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <Link to="/DepartamentoForm">
              {" "}
              {/* Enlace a la página de añadir Departamento */}
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-center">
                    <div className="font-semibold text-left">Facultad</div>
                  </th>
                  <th scope="col" className="px-6 py-3 text-center">
                    ...
                    <td className="px-6 py-4">
                      {
                        Facultad.find(
                          (Facultad) =>
                            Facultad.idFaculty === Departamento.idFaculty
                        )?.facultyName
                      }
                    </td>
                    <td className="px-6 py-4 text-left">
                      <Link
                        to={`/DepartamentoEdit/${Departamento.idDepartment}`}
                      >
                        {" "}
                      ...
                      <button
                        ...
                        onClick={() =>
                          setDepartamentoToDelete(Departamento.idDepartment)
                        } // Establece el ID de la Departamento para eliminar
                      >
                        Eliminar
                      </button>
                    </td>
